...
local Trace_with_Input_Layer = torch.class('tracenn.RNNTrace_with_Input_Layer')

function Trace_with_Input_Layer:__init(config)
  ...
  self.num_classes = 2
  -- Given the different training weight for two classes
  ...
  -- If the update_word_embedding Flag is on, add the embedding parameters of the input module
...
function Trace_with_Input_Layer:new_sim_module()
...
function Trace_with_Input_Layer:train(dataset, artifact)
  self.lrnn:training()
  self.rrnn:training()
  if string.starts(self.structure,'bi') then
  ...
  for i = 1, dataset.size, self.batch_size do
    ...
    local feval = function(x)
      ...
      for j = 1, batch_size do
        ...
        local linput_idx, rinput_idx

        -- load input artifact content using id
        if artifact.src_artfs_ids[dataset.lsents[idx]]~= nil then
          linput_idx = artifact.src_artfs[artifact.src_artfs_ids[dataset.lsents[idx]]]
        else
          print('Cannot find source:', dataset.lsents[idx])
          break
        end
        if artifact.trg_artfs_ids[dataset.rsents[idx]]~= nil then
          rinput_idx = artifact.trg_artfs[artifact.trg_artfs_ids[dataset.rsents[idx]]]
        else
          print('Cannot find target:', rsents[idx])
          break
        end

        local merged_index = torch.cat(linput_idx, rinput_idx,1)
        local sentences_after_mapping = self.input_module:forward(merged_index)

        -- get the left and right sentence after embedding layer
        local linputs, rinputs
        linputs = sentences_after_mapping:narrow(1,1, linput_idx:size(1))
        rinputs = sentences_after_mapping:narrow(1,linput_idx:size(1)+1, rinput_idx:size(1))

        -- get sentence representations after RNN layer
        local inputs
        if not string.starts(self.structure,'bi') then
        ...
        local linputs_grad, rinputs_grad
        if not string.starts(self.structure,'bi') then
          linputs_grad, rinputs_grad = self:RNN_backward(linputs, rinputs, rep_grad)
        elseif  string.starts(self.structure,'bi') then
          linputs_grad, rinputs_grad = self:BiRNN_backward(linputs, rinputs, rep_grad)
        end

        if self.update_word_embedding ==1 then
          -- backpropagate the gradient to the embedding layer
          local grad_for_input_model = torch.zeros(merged_index:size(1), linputs_grad:size(2))
          for i=1, linput_idx:size(1) do
            grad_for_input_model[i] = linputs_grad[i]
          end
          local count = 1
          for i=linput_idx:size(1)+1, grad_for_input_model:size(1) do
            grad_for_input_model[i] = rinputs_grad[count]
            count = count + 1
          end
...
function Trace_with_Input_Layer:RNN_backward(linputs, rinputs, rep_grad)
  local lgrad, rgrad
  if self.num_layers == 1 then
  ...
  local linputs_grad = self.lrnn:backward(linputs, lgrad)
  local rinputs_grad = self.rrnn:backward(rinputs, rgrad)
  return linputs_grad, rinputs_grad
end

-- Bidirectional LSTM backward propagation
function Trace_with_Input_Layer:BiRNN_backward(linputs, rinputs, rep_grad)
  ...
  local linputs_grad = self.lrnn:backward(linputs, lgrad)
  local linputs_grad_b = self.lrnn_b:backward(linputs, lgrad_b, true)
  local rinputs_grad = self.rrnn:backward(rinputs, rgrad)
  local rinputs_grad_b = self.rrnn_b:backward(rinputs, rgrad_b, true)
  return torch.add(linputs_grad, linputs_grad_b), torch.add(rinputs_grad, rinputs_grad_b)
end

-- Predict the similarity of a sentence pair (log probability, should use output:exp() for probability).
function Trace_with_Input_Layer:predict(lsent, rsent, artifact)
  self.lrnn:evaluate()
  self.rrnn:evaluate()
  ...
  if artifact.src_artfs_ids[lsent]~= nil then
    linput_idx = artifact.src_artfs[artifact.src_artfs_ids[lsent]]
  else
    print('Cannot find source:', lsent)
    return nil
  end
  if artifact.trg_artfs_ids[rsent]~= nil then
    rinput_idx = artifact.trg_artfs[artifact.trg_artfs_ids[rsent]]
  else
...
function Trace_with_Input_Layer:predict_dataset(dataset, artifact)
  local predictions = {}
...
function Trace_with_Input_Layer:predict_text(lsent, rsent)
  self.lrnn:evaluate()
  self.rrnn:evaluate()
  self.sim_module:evaluate()

  local merged_index = torch.cat(lsent, rsent,1)
  local sentences_after_mapping = self.input_module:forward(merged_index)
...
function Trace_with_Input_Layer:print_config()
...
function Trace_with_Input_Layer:save(path)
  local config = {
